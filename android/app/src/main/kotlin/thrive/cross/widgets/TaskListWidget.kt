...
class TickTaskAction : ActionCallback {
    companion object {
...
class TaskListWidget : GlanceAppWidget() {
    ...
    private fun Content(context: Context, state: HomeWidgetGlanceState) {
        ...
        val selectedFolder = prefs.getString("widget_task_folder", null)
            ?: folders.firstOrNull() ?: "Inbox"

        val filteredTasks = tasks.filter { it.folder == selectedFolder }
        ...
        Column(
            ...
            if (!isPro) {
                    ...
                    modifier = GlanceModifier
                        ...
                        .clickable(
                            actionRunCallback<LaunchAppAction>(
                                actionParametersOf(
                                    LaunchAppAction.uriKey to "cross://openPaywall"
                                )
                            )
                        ),
                ...
                LazyColumn {
                    items(filteredTasks.take(8)) { task ->
                        TaskRow(task)
                    }
                }
            }
    ...
    private fun TaskRow(task: WidgetTask) {
        Row(
            ...
            Text(
                ...
                style = TextStyle(
                    fontSize = 15.sp,
                    fontWeight = if (task.completed) FontWeight.Normal else FontWeight.Normal,
